...
export const CustomDrawer: React.FC<DrawerProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-end md:items-stretch justify-end p-0">
      ...
      <div
        className={cn(
          "relative z-10 w-full bg-white border-l border-border flex flex-col focus:outline-none shadow-modal transition-transform duration-250",
      ...
      >
        ...
        <div className="flex items-start justify-between p-6 pb-4 border-b border-gray-100">
          <div className="flex flex-col gap-1 pr-4">
            <h3 id="drawer-title" className="text-lg font-semibold text-gray-900 leading-none">
              {title}
            </h3>
            {description && (
        ...
        {/* Content (scrollable) */}
        <div className="flex-1 overflow-y-auto px-6 py-6 text-sm text-gray-700 leading-relaxed">
          {children}
        </div>

        {/* Footer */}
        {footer && (
          <div className="border-t border-gray-100 p-6 bg-gray-50/50 flex gap-3 justify-end">
            {footer}
          </div>
        )}
      </div>
